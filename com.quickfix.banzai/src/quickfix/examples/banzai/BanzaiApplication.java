...
public class BanzaiApplication implements Application {
    ...
    public void toAdmin(quickfix.Message message, SessionID sessionID) {
		...
		final Message.Header header = message.getHeader(); 
         StringField field = new StringField(MsgType.FIELD);
		try {
			if ( header.getField(field ).valueEquals( MsgType.LOGON ) ) { 

			    message.setField(new StringField(Username.FIELD, "quik")); 
			    message.setField(new StringField(Password.FIELD, "quik")); 
			 }
    ...
    public void send44(Order order) {
        quickfix.fix44.NewOrderSingle newOrderSingle = new quickfix.fix44.NewOrderSingle(
                new ClOrdID("1234566"), sideToFIXSide(order.getSide()),
                new TransactTime(), typeToFIXType(order.getType()));
        newOrderSingle.set(new OrderQty(order.getQuantity()));
        newOrderSingle.set(new Symbol(order.getSymbol()));
        newOrderSingle.set(new HandlInst('1'));
        newOrderSingle.set(new Account("86841"));
        newOrderSingle.set(new SecurityExchange("QJSIM"));
        send(populateOrder(order, newOrderSingle), order.getSessionID());
    }
